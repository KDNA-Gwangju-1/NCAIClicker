...
- 청구서는 동시에 한 장이다. 첫날 시작에 발행하고, 납부한 다음 날 새로운 청구서를 발행한다. 마감일은 발행일을 포함하며 이미 발행된 청구서는 단계 상승으로 바꾸지 않는다.
...
원작이 그렇게 한다. 플레이 중 화면에 뜨는 것은 좌상단 스태미나뿐이고, 코인·날짜·청구서·정확도는
**하루가 끝난 정산 화면에서** 본다.
...
- **파산은 프레스티지 진입점이 아니라 회차 실패로 축소한다.** 원작에서 파산은 회차를 끝내고 레거시 포인트를 정산하는 장치지만, 이중 화폐를 도입하지 않기로 했으므로 파산의 역할을 "회차 종료 + 보유 코인 초기화"로만 남긴다. **반액 정산 같은 별도 계수는 두지 않는다** — 4절 파산 절과 같은 규칙이며, 그래서 `bills.csv` 에도 정산 비율 항목이 없다. 청구서를 무시할 수 없게 만드는 압박은 유지하면서 새 시스템은 늘리지 않는다.

세부 실행 계획은 [EXECUTION_PLAN.md](EXECUTION_PLAN.md), 코드 아키텍처는 [ARCHITECTURE.md](ARCHITECTURE.md)를 참고한다.

## 10. 저장·재실행 경계

메뉴/결과의 거래·퍼크 선택과 하루 정산은 날짜·청구서·대출을 포함한 스냅샷으로 저장한다.
