...
- 다만 자동 망치만으로 버는 양이 적극적으로 움직이며 버는 양을 넘지 않도록 타격당 획득량을 낮게 잡는다. 넘어가는 순간 "커서를 세워두는 쪽이 이득"이 되어 게임을 플레이할 이유가 사라진다. 작업 7.2(밸런싱)에서 이 부등호를 실측으로 확인한다.
- **대상이 부서질 때** 코인이 쏟아져 바닥에 흩어진 채 남아 있다가, 런이 끝나는 시점에 **전량 자동으로 수금**된다. 플레이어가 커서로 주우러 다닐 필요는 없다.
