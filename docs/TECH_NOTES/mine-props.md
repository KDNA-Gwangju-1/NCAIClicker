...
- **렌더 큐는 2999 로 고정한다 (#316).** Transparent 기본 큐(3000)는 조준 레티클과 같은 큐라
  ...
  그리게 했다. **값은 인스펙터 Advanced > Sorting Priority(`_QueueOffset`) = -1 로 넣는다.**
  URP Lit 은 렌더 큐를 `Surface Type 기본 큐 + _QueueOffset` 으로 다시 계산하므로
  `Render Queue` 칸(`m_CustomRenderQueue`)만 2999 로 바꾸면 임포트 때 3000 으로 되돌아간다. Opaque 로 바꾸면 가장자리 페이드가 사라지므로 **Surface Type 은 Transparent 로 둔다.**
...
| 2026-09-23 | #316 | saltlake00 | 바닥이 레티클과 같은 투명 큐(3000)라 레티클을 덮던 문제를 `MineDirtFloor.mat` 의 Sorting Priority(`_QueueOffset`) -1 → 렌더 큐 2999 로 해결. 가장자리 페이드를 위해 Transparent 는 유지 |
