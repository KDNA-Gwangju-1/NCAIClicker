...
<table>
  ...
  <tr><td>2026.09.23</td><td>#322</td><td>soilrist</td><td>7.12 회차 날짜와 고지서 순번·기한이 섞여 읽히던 문제 — 날짜를 <code>회차 DAY N</code>, 고지서 줄을 <code>N번째 고지서 $N · 납부 기한 N일 남음</code>(완료 시 <code>N번째 고지서 납부 완료</code>)으로 바꿨다. 조기 납부로 '단계'가 '날짜'보다 빨리 오르면 둘이 같은 숫자 체계처럼 보였다. 프리팹 자리표시자는 런타임이 덮어쓰므로 그대로 둔다</td></tr>
</table>
