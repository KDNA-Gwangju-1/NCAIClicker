...
> 관련 이슈: #192, #196, #220 · 최종 수정: 2026.09.23
...
**설정 패널에서 저장을 초기화하면 이 패널이 런을 접고 메인 메뉴로 보낸다** (#220).
저장을 지워도 **이미 시작된 런은 되감기지 않기 때문**이다 — `BeginRun` 이 이미 돌아 그 판의
크리처·스태미나·피버가 살아 있는 채로 저장 쪽 상태(코인·성장·단계·날짜·고지서)만 새 회차 값이
되어 앞뒤가 맞지 않는다. 이 클래스가 맡는 이유는 **`Time.timeScale` 의
원래 값을 아는 것이 여기뿐**이어서다 — 설정 패널이 직접 씬을 넘기면 일시정지 중 0 인 채로
메인 메뉴가 열려 멈춰 보인다. 고른 이유와 버린 방법은 [설정 패널](settings-panel.md)
"런 도중 초기화" 에 있다.
...
* 설정 패널(#196) 조립 방식:
  ...
  주의(#220): PausePanelPrefabCreator 가 `PrefabUtility.InstantiatePrefab` 이 아니라
  `Object.Instantiate` 로 넣으므로 **중첩 인스턴스가 아니라 연결이 끊긴 사본**이다.
  SettingsPanel.prefab 을 고쳐도 여기는 따라 바뀌지 않는다 — 문구·치수를 고칠 때 양쪽을 같이 고친다.
...
* PausePanel.prefab
  ...
  역할: 1920x1080 불투명 단색 팔레트 UI 프리팹 (SettingsPanel **사본** 포함 — 중첩 인스턴스 아님)
...
* ResetPerformed (#220)
  발행: SettingsPanelController
  구독: PausePanelController (HandleSettingsReset)
  시점: 저장 초기화 직후. 받으면 timeScale 을 복원하고 MainMenu 로 보낸다.
  메인 메뉴에는 이 패널이 없어 구독자가 없다 — 그때는 설정 패널만 닫히는 것이 맞는 동작이라
  "런 중인가" 를 묻는 상태 검사가 필요 없다.

세 구독(OnStaminaChanged · Closed · ResetPerformed)은 OnEnable / OnDisable 에 쌍으로 있고
SetSettingsPanel 이 대상을 바꿀 때도 옛 대상에서 떼고 새 대상에 붙인다.
...
* 컴파일 에러 0건 확인 완료
* PausePanelUiChecks.RunBatch() 실행 통과 (총 21개 검사항목 PASS — #220 에서 15 → 21)
...
* 2026.09.21 (이슈 #220, 계획 6.16): 설정 패널의 저장 초기화가 런을 접고 메인 메뉴로 보내도록 ResetPerformed 구독 추가. 초기화 확인창 문구에 런 종료 안내를 넣고 3줄이 되며 넘치던 텍스트 칸을 확대(PausePanel.prefab). 검증 15 → 21건.
* 2026.09.23 (이슈 #220): Develop 32커밋 반영. #211·#221 이후 초기화가 날짜·고지서까지 되돌리므로 "단계만 0" 서술을 저장 쪽 상태 전부로 정정. 설정 패널이 중첩 인스턴스가 아니라 연결이 끊긴 사본이라는 점을 명시.
* 2026.09.23 (이슈 #220): 초기화 확인창 문구를 다시 고쳤다 — 지워지는 범위를 "반지까지 모든 진행"으로 넓히고, 메인 메뉴에서도 참이 되도록 "진행 중인 판이 있으면"으로 바꿨다. 3줄이 되어 09.21 에 키웠던 텍스트 칸·상자를 원래 치수로 되돌렸다.
