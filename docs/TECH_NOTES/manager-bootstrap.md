...
> 관련 이슈: #15, #80, #27, #26, #140, #150, #164, #203 · 최종 수정: 2026-09-21
...
- 붙어 있는 매니저는 `EconomyManager`·`SaveManager`·`StaminaManager`·`GameManager`·`FeverManager`·`BillManager`·`StageGoalManager`·`CreatureManager` 여덟 개다(경제 3.1, 저장 3.4, 스태미나·피버·런 상태 각 모듈, 고지서 마감 #27, 단계 목표 3.5, 크리처 스폰 #140). 매니저 사이의 연결(`EconomyManager.SetBillService`, `CreatureManager.SetUpgradeStats`, `CreatureManager.SetStageService`·`BillManager.SetStageService`, `SaveManager.SetPersistenceTargets` 등)은 `ManagerBootstrap`이 생성 직후 코드로 조립한다. 단계 주입은 `StageGoalManager`를 `IStageService`로 찾아 두 소비자에게 같은 인스턴스를 넘기는 방식이다(#150). 컴포넌트 실행 순서는 `GameManager.GetServiceOrder`를 통해 `Economy(1) -> Stamina(2) -> Fever(3) -> Creature(4) -> Stage(5) -> Bill(6)` 순으로 런 라이프사이클을 배선한다. **단계(5)가 고지서(6)보다 앞인 것은 지켜야 한다** — `EndRun` 에서 앞은 목표 달성 시 단계를 올리고 뒤는 파산 시 0 으로 되돌리기 때문이다(#164). 순번을 주지 않은 구현체는 기타(10)로 밀린다.
- **조립만 하는 곳이 아니다 — 저장 복원도 여기서 한 번 한다** (#203). `WirePersistence()` 가
  `SaveManager` 에 공급자 6개(`IEconomyService`·`IWalletPersistence`·`IUpgradePersistence`·
  `ILegacyService`·`ILegacyPersistence`·`IStageService`)를 주입한 **직후** `LoadAndDistribute()`
  를 부른다. 매니저는 `DontDestroyOnLoad` 라 씬을 다시 로드해도 값을 들고 있어서, 복원을
  씬 로드마다 하면 마지막 저장 이후의 변경이 덮어써진다 — 실제로 그런 회귀가 났다
  ([저장·불러오기](save-load.md) "배선"). `Instantiate` 가 `Awake` 를 이미 돌린 뒤이고 첫 씬은
  아직 로드되지 않아, 어느 `BeginRun` 보다도 앞선다.
...
| 2026-09-17 | #26 | soilrist | `StageGoalManager` 부착 반영, 프리팹에 붙은 매니저 7종(경제·저장·스태미나·게임·피버·고지서·단계 목표)을 실제 상태로 갱신 |
...
| 2026-09-18 | #164 | twins6375-art | `GetServiceOrder` 순번에 `Stage`(5)·`Bill`(6) 을 더한 것을 조립 설명에 반영. 단계가 고지서보다 앞이어야 하는 이유와 기타(10) 폴백을 적었다 |
| 2026-09-21 | #203 | twins6375-art | `WirePersistence()` 추가 — `SaveManager` 에 저장 대상 6개를 주입하고 이어서 복원을 **한 번** 실행한다. 조립에 더해 앱 시작 시 복원 시점까지 이 파일이 쥐게 됐다 |
