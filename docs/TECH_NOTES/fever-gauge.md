...
- **아무도 `BeginRun()` 을 부르지 않는다.** 작업 2.5 의 `GameManager` 가 들어왔지만 런 상태를
  추적만 하고 매니저를 시작시키지는 않는다. 스태미나도 같은 상태다 — **지금 Play 하면 게이지가
  움직이지 않는다.** 배선은 작업 2.6 첫 완주 빌드에서 이어져야 한다
...
- **퍼크 선택 중 일시정지가 없다.** BALANCE 5절이 "퍼크 선택 중에는 피버 시간도 멈춘다"고
  ...
  피버가 끝나 버린다
- **런 종료와 조회가 계약 밖이다.** `IRunScoped` 에는 `BeginRun()` 만 있다. 런 도중 구독을
  시작하는 UI 도 초기 게이지를 읽을 통로가 없다 (#111)
