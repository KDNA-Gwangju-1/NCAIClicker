...
**호버 망치가 맞힐 때마다 확률로 발동한다** (3.13). 발동하면 보유 수만큼 망치가 나와 살아 있는
대상을 하나씩 맡아 무조건 적중시켜 내구도를 깎고, 한 사이클(`auto_hammer_swing_sec`, 기본 1초)
뒤에 사라진다. 물리 판정을 쓰지 않고, 스태미나 소모 속도에도 영향을 주지 않는다.

**발동 중에는 확률을 굴리지 않는다.** 진행도를 되돌리면 망치가 장전만 반복하고 강타 지점에
영영 닿지 않는다. 자동 망치 자신의 적중도 세지 않는다 — 한 번 터진 발동이 다음 발동을 부르는
고리가 생긴다. 게임 규칙은 [GDD 4절](../GDD.md)에 있으니 여기서 반복하지 않는다.
...
| 단일 글로벌 타이머(주기 = 1 / 초당 타격 횟수), 틱마다 `수 × 파워`를 한 대상에 적용 | ⚠️ | 당시 GDD 공식과 일치했다. [#268](https://github.com/KDNA-Gwangju-1/NCAIClicker/issues/268) 에서 아래로 바꿨다 |
| **호버 적중 시 확률 발동**, 발동하면 한 사이클 동안 보유 수만큼 망치가 나와 때리고 사라진다 | ✅ | 팀장 지시 2026-09-22 (3.13). 자동 망치가 가만히 둬도 돌아가는 수입이 아니라 **플레이어의 타격에 얹히는 보너스**가 된다. GDD 4절 공식도 `호버 적중 수 × 발동 확률 × 수 × 파워` 로 함께 갱신했다. 확률은 퍼크가 올린다 |
...
| `IsSwinging` · `TickProgress` · `PendingTargetCount` · `TryGetPendingTargetPosition` | 없음 | 조회. `AutoHammerVisual` 이 매 프레임 읽는다 |
...
| `economy.csv` | `auto_hammer_proc_chance` | 호버 적중 1회당 발동 확률(%). `StatId.AutoHammerProcChance` 로 퍼크가 올린다 |
| `economy.csv` | `auto_hammer_swing_sec` | 발동 한 사이클(장전·강타·반동) 길이. 강타는 진행도 0.8 지점이다 |
...
| 2026-09-22 | #268 | yahoo-afk | 글로벌 타이머를 걷어내고 **호버 적중 시 확률 발동**으로 전환 (3.13, 팀장 지시). `economy.csv` 에 `auto_hammer_proc_chance` 신설, `auto_hammer_hits_per_sec` → `auto_hammer_swing_sec`(연출 길이) 로 대체. `StatId` 에 `AutoHammerProcChance` 추가 — 퍼크가 확률을 올릴 통로다. GDD 4절 공식도 함께 갱신했다 |
