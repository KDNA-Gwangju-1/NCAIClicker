...
> 관련 이슈: #196, #202, #203, #220 · 최종 수정: 2026-09-23
...
**무엇이 남는지는 그 뒤로도 바뀌었다.** 이 카드를 시작할 때는 날짜·고지서가 수집 대상이 아니라
그대로 남는 것이 문제의 절반이었는데, #211·#221 이 `IBillPersistence` 와 `RestoreCycle` 을 붙여
그쪽은 함께 초기화된다. 그래도 **이미 시작된 런은 되감기지 않는다** — `BeginRun` 이 이미 돌아
크리처·스태미나·피버가 그 판의 것으로 살아 있는 채로 저장 쪽 상태(코인·업그레이드·레거시 포인트·반지·단계·날짜·고지서·대출)만
새 회차 값이 된다.
...
| 런 상태까지 함께 초기화하고 런은 계속한다 | ❌ | 날짜·고지서는 #221(`IBillPersistence`) 이후 실제로 함께 초기화된다. 그래도 **이미 시작된 런은 되감을 수 없다** — `BeginRun` 이 이미 돌아 크리처·스태미나·피버가 그 판의 것으로 살아 있다. 되감으려면 런 경계를 다시 태워야 하는데 그것은 `GameManager` 만 할 수 있는 일이다 |
| 초기화하면 런을 접고 메인 메뉴로 보낸다 | ✅ | "초기화"의 뜻에 가장 가깝다. 런을 통째로 버리면 **되감을 것이 남지 않는다** — 위 방법이 걸리는 지점을 우회하는 것이 아니라 없애는 쪽이다 |
...
| (프리팹) | `Assets/Prefabs/UI/SettingsPanel.prefab` | 위 컨트롤러가 붙은 패널 프리팹. `MainMenuPanel.prefab` 이 **중첩 인스턴스**로 품고(원본을 고치면 따라 바뀐다), `Resources/UI/PausePanel.prefab` 은 **연결이 끊긴 사본**을 따로 들고 있다(따라 바뀌지 않는다) |
...
| 설정 → 초기화 → 확인창 | 문구 정상 (최종 문구·치수는 아래 "확인창 문구") |
...
- [x] **확인창 문구** — 최종본은 아래이고, 칸·상자 치수는 `Develop` 그대로(칸 100 · 상자 280)다.

      > 정말 초기화할까요?
      > 반지까지 모든 진행이 사라지며 되돌릴 수 없습니다.
      > 진행 중인 판이 있으면 끝나고 메인 메뉴로 돌아갑니다.

      Play Mode 로 인게임(`PausePanel` 사본)·메인 메뉴(`MainMenuPanel` 중첩) 양쪽에서 쟀고
      **3줄 · 필요 83.17 · 배정 100 · 넘침 없음**이었다.

      여기까지 세 번 바꿨다. (1) 런 종료 안내를 한 줄 덧붙였더니 칸을 11px 넘겨 칸 120·상자 300 으로
      키웠다. (2) 검수가 두 가지를 짚었다 — 원래 문구 "업그레이드와 납부 기록이 모두 사라지고" 는
      **실제로 지워지는 범위보다 좁다**(코인·레거시 포인트·반지·단계·날짜·대출도 지워진다. 특히 반지는
      파산해도 남는 영구 층이라 플레이어가 안전하다고 여길 만하다). 그리고 "진행 중인 판도 여기서
      끝나고 메인 메뉴로 돌아갑니다" 는 **메인 메뉴에서 열면 거짓**이다 — `MainMenuPanel` 이 같은
      프리팹을 중첩해 똑같이 뜨는데 거기엔 끝날 판이 없다. (3) 둘을 고치자 5줄로 넘쳐 문구를 줄였다.
      "반지까지" 가 가장 놀랄 만한 손실을 짚으면서 나머지를 덮고, "있으면" 으로 두 화면 모두에서
      참이 된다. 3줄이 되어 **원래 치수로 되돌릴 수 있었고**, 남의 프리팹에 남는 변경은 문구 한 줄이다.
      (넘칠 때는 `overflowMode` 가 `Overflow` 라 잘리지 않고 **아래 버튼과 겹치는** 형태였다)
- [x] 전체 검증 회귀 (2026-09-21 시점): `ValidationRunner.RunAll()` — 통과 24 / 실패 1 (전체 25).
      실패는 `TargetChecks` 하나였고 이 작업과 무관했다 (6.32/#287 에서 고쳐졌다).
      **`PASS` 줄만 읽으면 실패가 보이지 않는다 — 요약 줄을 확인할 것.** 실제로 그래서
      "전 스위트 통과" 라고 잘못 적었다가 정정했다 (아래 재검증에서 26/26 으로 확인)
#### 재검증 (2026-09-23, Develop 32커밋 반영 후)

#211·#221 이 `ResetAndDistribute`·`LoadAndDistribute` 를 고친 뒤 다시 쟀다. 저장에 코인 7000·
포인트 150·반지 1·업그레이드 `[2,0,0,0]`·단계 2·**5일차·고지서 3·회차 2**·설정
`Bgm 0.25 / 흔들림 꺼짐` 을 심고 `Game` 씬에서 시작.

| 단계 | 결과 |
|---|---|
| 앱 시작 복원 | 코인 7000 · 단계 2 · 포인트 150 · **6일차**(5 + `BeginRun` 증가) · 고지서 235 활성 |
| ESC → 설정 → 초기화 직전 | `timeScale` 0 |
| "예, 초기화한다" | `timeScale` **1 복구** · `IsPaused` false · 코인 0 · 단계 0 · 포인트 0 · **날짜 1 · 고지서 없음** |
| 다음 프레임 | **씬 = MainMenu** · 저장 파일 전부 0 · **회차 1 로 리셋** · 설정 `0.25 / 꺼짐` 보존 |
| 메인 메뉴 경로 | 일시정지 패널 없음(구독자 0) → 씬 유지 · 패널만 닫힘 · `timeScale` 1 · 메모리 코인 0 |

- [x] **날짜·고지서가 함께 초기화된다** — #221 이전에는 그대로 남던 부분이다. 이 카드를 시작할 때
      적었던 "고지서·날짜는 수집 대상이 아니라 그대로 남는다" 는 이제 사실이 아니며, 그래도
      **이미 시작된 런은 되감기지 않아** 이 카드의 결론은 그대로다
- [x] 전체 검증: `ValidationRunner.RunAll()` — **통과 26 / 실패 0 (전체 26)**. 요약 줄로 확인했다.
      **도메인 리로드 직후에 잰 값이다.** 같은 에디터 세션에서 런타임 스크립트를 여러 번 고쳐
      재컴파일한 뒤 돌렸을 때 한 번 **통과 15 / 실패 11** 이 나왔는데, 실패가 전부 정확히 두 배
      (지급액·누적량·포인트)라 정적 이벤트 이중 구독의 증상이었다. 파괴된 `CreatureManager` 가
      `OnTargetBroken` 에 매달려 있었다. `RequestScriptCompilation()` 으로 도메인을 비운 뒤에는
      26/26 이었고, 이 카드의 검사를 3회 돌린 뒤 다시 돌려도 26/26 이라 **이 검사가 오염원은
      아니다.** 결과가 이상하게 나오면 먼저 도메인을 비우고 다시 잴 것
- [x] 콘솔 오류 0건
...
| 2026-09-23 | #220 | twins6375-art | Develop 32커밋 반영 후 근거 정정 — #211·#221 이후 초기화가 날짜·고지서·대출까지 되돌리므로 "고지서·날짜는 그대로 남는다"·"단계만 0" 서술을 저장 쪽 상태 전부로 고쳤다. 남는 이유는 "이미 시작된 런은 되감기지 않는다" 하나. 재검증 26/26, 확인창 사본 셋 중 하나가 중첩 인스턴스임을 명시. 확인창 문구가 메인 메뉴에서 거짓이 되던 것과 초기화 범위를 좁게 적던 것을 고쳤다 |
