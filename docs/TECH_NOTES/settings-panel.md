...
- [x] 전체 검증 회귀 (2026-09-21 시점): `ValidationRunner.RunAll()` — 통과 24 / 실패 1 (전체 25).
      ...
      **`PASS` 줄만 읽으면 실패가 보이지 않는다 — 요약 줄을 확인할 것.** 실제로 그래서
      "전 스위트 통과" 라고 잘못 적었다가 정정했다
- [ ] **재검증 필요** — 위 Play Mode·검증 결과는 `Develop` 이 32커밋 앞서기 **전**에 잰 것이다.
      그 사이 #211·#221 이 `ResetAndDistribute`·`LoadAndDistribute` 를 고쳤으므로
      (날짜·고지서가 이제 함께 초기화된다) 머지 전에 다시 재야 한다
