...
> 관련 이슈: #196, #202 · 최종 수정: 2026-09-21
...
- **저장 초기화(`HandleResetConfirmed`)는 디스크의 저장 파일만 새로 쓴다.** 현재 실행 중인 `EconomyManager` 등 메모리 상태는 초기화하지 않는다 — [save-load.md](save-load.md)에 이미 기록된 "자동 로드/저장 배선이 없다"는 한계와 같은 선상이다. 재시작(다음 `Load()`) 시점에만 초기화된 값이 실제로 반영된다.
