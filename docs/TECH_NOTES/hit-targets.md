...
크리처는 책상 안전 영역(경계) 안을 배회하며, 피격 시 경직(BeingHit) 후 도망(Fleeing)치고, 파괴된 자리는 일정 시간 뒤 다시 스폰된다.
...
| `CreatureMovementChecks` | `Assets/Scripts/Editor/CreatureMovementChecks.cs` | 이동, FSM 전이, 경계 클램프, 스폰 계산 검증 11건 |
