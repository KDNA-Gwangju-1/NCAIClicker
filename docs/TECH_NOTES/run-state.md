...
스태미나 소진·파산 이벤트로 결정한다. 게임 규칙은 [GDD](../GDD.md)에 있으니 여기서 반복하지 않는다.
...
| 클래스 | 경로 | 하는 일 |
|---|---|---|
| `GameManager` | `Assets/Scripts/Runtime/Core/GameManager.cs` | 런 상태 소유. 씬 로드로 `MainMenu`/`Running`을, 이벤트로 `Result`를 결정한다. `Managers` 프리팹(Resources)에 부착 |
...
Unity MCP로 Play Mode에 직접 진입해 확인했다 (자동 테스트 아님, 아래 "알려진 한계" 참고).
...
- [x] `GameEvents.PublishStaminaDepleted()` 강제 발행(Running 상태에서) → `CurrentState == Result`
- [x] `SceneManager.LoadScene("MainMenu")` 호출 → `CurrentState == MainMenu`로 복귀
- [x] `Game` 씬을 처음부터 직접 열고 Play(개발자가 자주 쓰는 경로) → `Awake` 초기값이 `CurrentState == Running`으로 정상 계산됨
- [ ] `GameEvents.PublishBankrupt()` 로 별도 검증 (스태미나 소진과 동일한 핸들러를 타므로 코드 경로는 같지만, 별도로 발행해 확인하지는 않았다)
...
- `EconomyManager.BeginRun()`/`RestoreWallet()` 호출을 GameManager가 아직 연결하지 않았다. 두 메서드는 `IEconomyService`에 없는 `EconomyManager` 전용 public 메서드라 인터페이스로 호출할 수 없다 — [#71](https://github.com/KDNA-Gwangju-1/NCAIClicker/issues/71) (`IEconomyService`에 런 경계 통로 추가)이 닫힌 뒤 별도로 연결해야 한다.
- 청구서 마감·납부·대출·파산 판정 등 "하루 종료 순서"(ARCHITECTURE.md 2절) 전체 오케스트레이션은 구현하지 않았다. 이번 이슈는 `Result` 전이만 담당하며, 나머지는 4.x 이슈들 몫이다.
- 자동 테스트가 없다. `NCAIClicker.Tests.PlayMode.asmdef`가 런타임 코드를 참조하지 않는 구조([#80](https://github.com/KDNA-Gwangju-1/NCAIClicker/issues/80) 범위)라, `GameManager` 타입을 직접 쓰는 테스트를 추가하지 않고 수기 Play Mode 검증으로 대체했다.
