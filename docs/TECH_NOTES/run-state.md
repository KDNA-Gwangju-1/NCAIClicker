...
**단계(5)가 고지서(6)보다 먼저**여야 한다. `EndRun` 에서 앞은 목표 달성 시 단계를 올리고 뒤는
...
- ~~고지서 마감·납부·대출·파산 판정 등 "하루 종료 순서" 전체 오케스트레이션은 구현하지 않았다.~~ — #164 에서 `BillManager` 가 `IRunScoped` 를 구현하며 런 경계에 붙었다. 하루 진행·고지서 발행·대출 징수·파산 판정이 실제로 돈다.
