...
| `IGamePersistence` | `Assets/Scripts/Runtime/Interfaces/ISaveService.cs` | 매니저 상태 수집·분배·초기화(`CollectAndSave`/`LoadAndDistribute`/`ResetAndDistribute`). `SaveManager` 구현, `SaveManager.Persistence` 가 이 타입으로 노출. **런타임 소비처는** `ManagerBootstrap`(복원 1회)·`GameManager`(저장·새 회차 초기화)·`SettingsPanelController`(저장 초기화, #196) 셋이다 (이슈 #203) |
