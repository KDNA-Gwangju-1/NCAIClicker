...
| `IUpgradePersistence` | `Assets/Scripts/Runtime/Interfaces/IEconomyService.cs` | 업그레이드 레벨 저장 복원. SaveManager 전용 (이슈 #116, 배선 #203) |
...
| `IGamePersistence` | `Assets/Scripts/Runtime/Interfaces/ISaveService.cs` | 매니저 상태 수집·분배·초기화(`CollectAndSave`/`LoadAndDistribute`/`ResetAndDistribute`). `SaveManager` 구현, `SaveManager.Persistence` 가 이 타입으로 노출. **런타임 소비처는** `ManagerBootstrap`(복원 1회)·`GameManager`(저장·새 회차 초기화)·`SettingsPanelController`(저장 초기화, #196)·`BillManager`(납부 후 흐름 즉시 저장, #249) 넷이다 (이슈 #203) |
...
| `IStageService` | `Assets/Scripts/Runtime/Interfaces/IStageService.cs` | 단계 진행 상태 공용 조회(`CurrentStageIndex`/`CurrentStageNumber`/`IsStageCleared`/`IsMaxStage`/`AdvanceStage`/`RestoreStage`). `StageGoalManager` 구현, `ManagerBootstrap`이 `CreatureManager`·`BillManager`·`SaveManager`에 주입 (이슈 #150, #203) |
...
* ~~#116: `IUpgradePersistence`는 계약과 `EconomyManager` 구현만 있고 `SaveManager`가 호출하지 않는다~~ — #203 에서 배선했다. `ManagerBootstrap`이 `SaveManager.SetPersistenceTargets`로 주입하고, `SaveManager.CollectAndSave`/`LoadAndDistribute`가 부른다.
* ~~#175: **`ILegacyPersistence`도 같은 상태다.**~~ — #203 에서 `IUpgradePersistence`와 **같은 방식으로** 함께 배선했다. 한쪽만 연결하면 방식이 두 벌 생기므로 한 이슈로 묶었다 ([레거시 포인트와 반지](legacy-points.md)).
...
* #203: 고지서·대출·퍼크 후보·`LastRunCoin`·`WasBankrupt`·`ResumePoint`·`BestRunCoin` 은 아직 수집하지 않는다. `IBillService`에 복원 통로가 없어 담아도 되돌릴 수 없고, 쓰기만 하고 읽지 못하는 필드는 "저장된다"는 착각만 만든다 ([고지서](billing.md) 알려진 한계). 계약 이슈가 먼저다.
...
| 2026-09-23 | #291 | twins6375-art | `IBillService` 에 `bool IsPrestigeWindowOpen { get; }` 추가 (공용 계약 변경, #291 에서 방식 합의). 기존 시그니처는 그대로라 구현체는 속성 하나만 더하면 된다 — `BillManager` 와 가짜 둘(`BillPanelChecks`·`EconomyManagerChecks`). `EconomyManager.TryPurchaseRing` 이 소비한다. 저장 스키마는 바꾸지 않았다 |
