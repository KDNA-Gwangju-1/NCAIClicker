...
온다. 한 런이 75초면 하루가 길어 마감 압박이 옅어지고, 청구서 한 장을 넘기는 데 드는 실제
...
- `stages.csv` 의 목표 코인과 `bills.csv` 의 청구서 금액·기한을 이 길이에 맞춰 다시 뽑아야 한다.
  다시 뽑기 전까지 **첫 청구서 납부 가능 여부는 검증되지 않은 상태다.**
