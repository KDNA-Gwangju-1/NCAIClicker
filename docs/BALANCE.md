...
> ## ❗ 이 절의 계산은 무효다 — 재산출 필요 (킥오프)
>
> 아래 계산은 **타격 대상 3종 시절, 타격마다 코인을 주던 시절**의 것이다. 그 뒤 두 가지가 바뀌었다.
>
> | 바뀐 것 | 영향 |
> |---|---|
> | 회복형(`tourist`) 추가로 4종이 됨 | 1단계 평균 코인 배율 **1.08 → 0.93** (`stages.csv` 기준: 일반 60 / 거치 15 / 고속 10 / 회복 15) |
> | 코인을 **파괴 시에만** 지급하도록 변경 | 타격 횟수가 아니라 **몇 마리를 끝까지 부쉈는가**가 수입을 정한다. 계산 모델 자체가 다르다 |
> | 회복형이 스태미나 8 을 돌려줌 | 런 길이가 고정이 아니다. 한 마리당 약 5초, 스윙 20회가 늘어난다 |
>
> **방향이 서로 반대라 어림으로 메울 수 없다.** 배율 하락과 파괴 판정은 수입을 깎고, 스태미나
> 회복은 늘린다. 킥오프에서 실제 값으로 다시 계산한 뒤 1단계 목표(현재 500)를 조정한다.
>
> 아래는 **모델의 형태를 보여주는 참고용**으로만 남긴다.
...
업그레이드를 하나도 안 산 첫 런에서 **548 대 500**으로 겨우 넘긴다 — **이 결론은 위 사유로 더 이상
성립하지 않는다.** 노리는 난이도는 그대로다: 첫 런에 아슬아슬하게 넘고, 표적을 놓치는 시간이
길면 미달하되 코인은 남아 업그레이드를 사서 재도전하게 된다.
...
지금 수치는 **의도적으로 낮게 잡았다.** 대출과 파산이 붙은 상태에서 청구서까지 무거우면 첫 플레이가 좌절스럽다. 작업 7.2(밸런싱)에서 실측한 뒤, 마감일 수입의 **25~35%** 가 되도록 금액을 끌어올린다. 그 구간을 넘으면 납부하는 순간 업그레이드를 살 돈이 없어 성장이 멈추고, 훨씬 아래면 그냥 다 내면 되니 판단이 사라진다.
...
대출은 마감일에 돈이 모자랄 때만 의미가 있다. 이자 10% 에 매일 수입의 5~10% 를 징수당하므로, **하루 수입의 절반을 넘는 금액을 빌리면 상환 자체가 어려워진다.** 작업 7.2(밸런싱)에서 "빌려서 살아남은 뒤 실제로 갚아지는가"를 실측한다.
...
> **이 값들을 실제로 어떻게 적용할지는 성장·저장 담당이 정한다.** CSV와 `BalanceData` 는 "무엇을 얼마나 바꾼다"는 선언만 담고, 계산 순서나 적용 시점은 구현자의 영역이다.
...
| `bills.csv` | 성장·저장 | 마감일·대출 계수. 변경 시 4절 조건을 임포터가 자동 검사 |
| `fever.csv` | 피버·보너스 | |
...
(`BalanceImporter.cs` 의 `st.DueDays > 0 ? st.DueDays : d.Bill.DueDays`). 단계 데이터가 비어 있을 때만
기본값이 쓰인다.

**모든 수치는 작업 7.2(밸런싱)에서 실측으로 검증한다.** 위 계산은 가정(호버 유지율 60%, 이동 속도 3유닛/초)에 기댄 값이라, 실제 플레이 로그와 다르면 가정 쪽을 먼저 고친다.
