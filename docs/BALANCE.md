...
`idle_drain_per_sec` 를 **1.6 에서 6.5 로 올렸다.** 기본 런은 `120 / 6.5 = 18.5초`다.

**바꾼 이유**: 원작([Bills Must Be Paid](https://store.steampowered.com/app/4421010/_Bills_Must_Be_Paid/))의
스태미나 소모 속도에 맞췄다. 이전 값 1.6 은 자체 시연 목표(60~90초)에서 역산한 것이라
원작 관찰값이 아니었다 — `max_stamina` 주석이 "원작 최초 기본 최대치로 확인된 값은 아니다"라고
적어 둔 것과 같은 사정이다.
