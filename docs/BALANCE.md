...
### 런 길이 검증 (2026-09-16, #5) — 폐기, `idle_drain_per_sec` 1.6 기준

**아래 표는 옛 값 기준이다. 현재 CSV(6.5)에는 적용되지 않는다.** 재측정 전까지 근거로 쓰지 않는다.
방법(각 2,000회, `--policy random`, 성공률을 바꿔가며 상한 확인)은 그대로 재사용할 수 있어 남겨 둔다.

당시 기본 길이는 `120 / 1.6 = 75.0초`였다. 회복형 파괴가 얹혀 실제 런은 더 길어지므로,
