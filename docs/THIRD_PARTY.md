...
| 광물 크리처 5종 3D 모델 (철·구리·은·금·다이아몬드) | 3D 모델 | VARCO 3D (https://3d.varco.ai, NC AI 생성형 AI 도구) | 위와 동일 (개인/비상업 라이선스, 팀 계정 교육 용도 허용) | https://terms.varco.ai (전문 미인용, 요약만 기록) | 타격 대상 4종 시각 에셋, 저금통 테마를 대체 (`Assets/Models/MineCreatureCopper.glb`, `MineCreatureSilver.glb`, `MineCreatureGold.glb`, `MineCreatureDiamond.glb`, `MineCreatureIron.glb`). 광산 작업장 컨셉아트에 맞춰 2026-09-22 재생성(#234), 철은 2026-09-23 추가(#282) — 구 `MineralCreature*.glb` 세트를 대체, 자세한 경위는 [mineral-creature-assets.md](TECH_NOTES/mineral-creature-assets.md) 참고 |
...
| 환경 대형 3종 3D 모델 (갱도 입구·목책·자수정 군락) | 3D 모델 | VARCO 3D (https://3d.varco.ai, NC AI 생성형 AI 도구) | 위와 동일 (개인/비상업 라이선스, 팀 계정 교육 용도 허용) | https://terms.varco.ai (전문 미인용, 요약만 기록) | 씬 가장자리 대형 환경 에셋 (`Assets/Models/TunnelEntrance.glb`, `FenceSection.glb`, `AmethystCluster.glb`). 2026-09-22 생성(#236) — 갱도 입구는 원본 참고 이미지의 깊은 원근감 때문에 이미지→3D 변환이 반복적으로 뒤틀려, 입구 안쪽을 평평한 벽으로 표현하도록 이미지를 보정한 뒤 재생성. 자세한 경위는 [mine-props.md](TECH_NOTES/mine-props.md) 참고 |
| 컨테이너 소품 4종 3D 모델 (광차·나무상자·자루·나무통) | 3D 모델 | VARCO 3D (https://3d.varco.ai, NC AI 생성형 AI 도구) | 위와 동일 (개인/비상업 라이선스, 팀 계정 교육 용도 허용) | https://terms.varco.ai (전문 미인용, 요약만 기록) | 씬 가장자리 소품 시각 에셋 (`Assets/Models/OreCart.glb`, `WoodenCrate.glb`, `OreSack.glb`, `Barrel.glb`). 2026-09-23 생성(#237) — 광차는 바퀴 방향이 제각각이던 결과를 바퀴 정렬을 명시한 새 이미지로 재생성, 나무상자는 반복된 메시 구멍·왜곡 문제로 단순화한 이미지에서 시작해 여닫이 뚜껑·광산 느낌을 다시 입혀 재생성, 자루는 원본 이미지에 박힌 곡괭이 엠블럼을 EditImage로 제거. 자세한 경위는 [mine-props.md](TECH_NOTES/mine-props.md) 참고 |
