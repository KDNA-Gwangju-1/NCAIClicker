...
| 진행 중 (착수 대상 아님) | 남이 잡고 있다. 보고에 넣되 착수 후보로 올리지 않는다 |
| 보드에 없는 열린 이슈 | 보고해서 카드를 올리게 한다 |
| 합계 | 카드가 빠짐없이 분류됐는지 확인하는 줄 |

## 모든 줄에 작성자를 함께 찍는다

덩어리를 가리지 않고 **모든 이슈 줄은 작성자(`작성 <로그인>`)를 포함한다.** 담당자와 작성자는
다른 사람일 수 있고, 착수 가능 카드는 애초에 담당자가 비어 있다. 작성자가 없으면
"이 이슈 이렇게 적힌 게 맞냐"를 물을 곳이 목록에 없다.

GitHub Projects 보드에는 **작성자 필드가 없다** (내장 필드가 아니다). 커스텀 필드로 흉내 내면
손으로 채워야 해서 금방 낡는다. 그래서 보드가 아니라 이 목록이 작성자를 보여주는 자리다.

## 목록에 없는 이슈의 상태를 추측하지 않는다

합계 줄의 수가 맞는데도 찾는 이슈가 안 보이면 **완료됐다는 뜻이다** — 완료 카드는
어느 덩어리에도 올라오지 않는다. 그래도 확인은 `gh issue view <번호>` 로 한다.

추측하면 틀린다. 실제로 방금 닫힌 이슈를 "목록에 없으니 진행 중이겠다"로 읽고
없는 병목을 보고한 적이 있다. `진행 중` 덩어리와 합계 줄은 그 추측을 막으려고 있는 것이니
**둘을 지우거나 보고에서 빼지 마라.**
